...
function MomentPlot({ 
  ...
  const y = d3.scaleLinear(d3.extent(data, d => d.moment), [height-marginBottom, marginTop]).nice();
  const line = d3.line()
    .x(d => x(d.beamLength))
    .y(d => y(d.moment));
    ...
    {(tooltipData !== null) && (
      ...
      >
        ...
        <text style={{
          ...
          <tspan x="1em" y="2.2em">{`Moment: ${tooltipData.moment.toFixed(3)} k-ft`}</tspan>
        </text>
      </Tooltip>
    )}
    </svg>
