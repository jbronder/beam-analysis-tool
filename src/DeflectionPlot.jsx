...
function DeflectionPlot({ 
  ...
  const y = d3.scaleLinear(d3.extent(data, d => d.deflection), [height-marginBottom, marginTop]).nice();
  const line = d3.line()
    .x(d => x(d.beamLength))
    .y(d => y(d.deflection));
      ...
      {(tooltipData !== null) && (
        ...
        >
          ...
          <text style={{
            ...
            <tspan x="1em" y="2.2em">{`Deflection: ${tooltipData.deflection.toFixed(4)} in`}</tspan>
          </text>
        </Tooltip>
      )}
    </svg>
